...
const EmployeeEdit = ({ employee, onSave, onCancel }) => {
    ...
    useEffect(() => {
        if (employee) {
            // Format dates if they are in ISO format to match the YYYY-MM-DD format of date inputs
            const formatDate = (dateString) => {
                if (!dateString) return '';
                // Handle ISO format (T) and SQL format (space)
                let date = dateString;
                if (typeof dateString === 'string') {
                    date = dateString.split('T')[0].split(' ')[0];
                }
                return date;
            };
